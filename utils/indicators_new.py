...
class TechnicalIndicators:
    ...
    def calculate_sma(df: pd.DataFrame, period_1: int = 20, period_2: int = 80) -> pd.DataFrame:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            df_new["INDICADOR_1"] = future_1.result()
            df_new["INDICADOR_2"] = future_2.result()
    ...
    def calculate_macd(df: pd.DataFrame, short_period: int = 12, long_period: int = 26,
        ...
        df_loc['INDICADOR_1'] = ema_short - ema_long
        df_loc['INDICADOR_2'] = df_loc['INDICADOR_1'].ewm(span=signal_period, adjust=False).mean()
    ...
    def calculate_ichimoku(df):
        ...
        with ThreadPoolExecutor() as executor:
            ...
            tenkan = f1.result()
            kijun = f2.result()
            df_new['INDICADOR_1'] = tenkan
            df_new['INDICADOR_2'] = kijun
            df_new['INDICADOR_3'] = ((tenkan + kijun) / 2).shift(26)
            df_new['INDICADOR_4'] = f3.result()

        df_new['INDICADOR_5'] = df_new['Close'].shift(-26)
    ...
    def calculate_bollinger_bands(df, window=20, num_std_dev=2):
        ...
        df_new['INDICADOR_1'] = sma
        df_new['INDICADOR_2'] = sma + (std * num_std_dev)
        df_new['INDICADOR_3'] = sma - (std * num_std_dev)
    ...
    def calculate_atr(df: pd.DataFrame, period: int = 14) -> pd.DataFrame:
        ...
        df_new['INDICADOR_1'] = df_new['TR'].ewm(span=period, adjust=False).mean()
    ...
    def calculate_keltner_channels(df, ema_period=20, atr_period=10, multiplier=2):
        ...
        df_new['INDICADOR_2'] = ema
        df_new['INDICADOR_1'] = ema + multiplier * atr
        df_new['INDICADOR_3'] = ema - multiplier * atr
    ...
    def calculate_donchian_channel(df, period=20):
        df_new = df.reset_index()
        df_new['INDICADOR_1'] = df_new['High'].rolling(window=period).max()
        df_new['INDICADOR_2'] = df_new['Low'].rolling(window=period).min()
        df_new['INDICADOR_3'] = (df_new['INDICADOR_1'] + df_new['INDICADOR_2']) / 2
    ...
    def calculate_supertrend(df, period=7, multiplier=3):
        ...
        df_new['INDICADOR_1'] = basic_upper
        df_new['INDICADOR_2'] = basic_lower
    ...
    def calculate_rolling_fibonacci(df, period=144):
        ...
        df_new['FIB_236'] = roll_max - (0.236 * diff)
        df_new['FIB_382'] = roll_max - (0.382 * diff)
        df_new['FIB_500'] = roll_max - (0.500 * diff)
        df_new['FIB_618'] = roll_max - (0.618 * diff) 
